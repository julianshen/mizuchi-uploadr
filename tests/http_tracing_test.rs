...
async fn test_w3c_trace_context_extraction() {
    ...
    let request = mizuchi_uploadr::server::http_tracing::MockRequest {
        method: "PUT".to_string(),
        path: "/uploads/test.txt".to_string(),
        headers: vec![("traceparent".to_string(), traceparent.to_string())],
    };
    ...
    assert_eq!(ctx.trace_state, None); // No tracestate in this test
}

/// Test that tracestate header is extracted
#[cfg(feature = "tracing")]
#[tokio::test]
async fn test_tracestate_extraction() {
    let config = TracingConfig {
        enabled: true,
        service_name: "test-service".to_string(),
        otlp: Default::default(),
    ...
    // Create request with both traceparent and tracestate
    let traceparent = "00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01";
    ...
    let request = mizuchi_uploadr::server::http_tracing::MockRequest {
        ...
        headers: vec![
            ...
            ("tracestate".to_string(), tracestate.to_string()),
        ],
    };

    let context = mizuchi_uploadr::server::http_tracing::extract_trace_context(&request);

    // Verify tracestate was extracted
    assert!(context.is_some());
    let ctx = context.unwrap();
    assert_eq!(ctx.trace_state, Some(tracestate.to_string()));
}
