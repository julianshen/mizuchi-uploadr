...
async fn test_jwt_span_no_pii() {
    ...
    assert_eq!(
        attributes.get("auth.token_present"),
        Some(&"true".to_string())
    );
...
async fn test_authz_span_no_pii() {
    ...
    // Resource type should be sanitized (only "uploads", not "sensitive.txt")
    assert_eq!(
        attributes.get("authz.resource_type"),
        Some(&"uploads".to_string())
    );
    assert!(!attributes.values().any(|v| v.contains("sensitive")));
}
